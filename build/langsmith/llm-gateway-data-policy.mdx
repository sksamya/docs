...
Every call through the LLM Gateway is traced to LangSmith. Trace content (request and response bodies) is off by default for all organizations, and a data policy turns it on for the requests that policy matches. When content capture is off, the gateway still records metadata: cost, token counts, timing, and policy evaluation results. Traces without content are not billed as traces.
