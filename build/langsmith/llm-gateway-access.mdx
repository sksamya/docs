...
Trace content (request and response bodies) is off by default. Turn it on with a [data policy](/langsmith/llm-gateway-data-policy#trace-content), which applies to any traces emitted through the gateway that match the scope of the policy.
